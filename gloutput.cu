...
void add_frame_tomovie (GLubyte **pixels) {
    frame->pts = nframes;
...
void init_movie(char *filename)  {
    int glget;
    
    time0 = glutGet(GLUT_ELAPSED_TIME);
    //model_init();
#if FFMPEG
    ffmpeg_encoder_start(filename, CODEC_ID_MPEG1VIDEO, 25, WIDTH, HEIGHT);
#endif
}
...
static void init(void)  {
    ...
    model_init();
#if FFMPEG
    ffmpeg_encoder_start("tmp.mpg", CODEC_ID_MPEG1VIDEO, 25, WIDTH, HEIGHT);
#endif
}
