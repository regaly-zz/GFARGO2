...
static double OldRadii[MAX1D], OldRmed[MAX1D], New_r[MAX1D];
...
//  if (!CPU_Master) return;
  ...
  if (RAD == NULL) return;
  fscanf (DIM,"%d %d %d %d %lf %d %d %d\n",&foo,&foo,&foo,&foo,&Foo,&foo,&OldNRAD,&OldNSEC);
  for (i = 0; i <= OldNRAD; i++) {
    fscanf (RAD, "%lf", &value);
    OldRadii[i] = (double)value;
  }
  fclose (DIM);
  fclose (RAD);
  for (i = 0; i < OldNRAD; i++) {
...
void CheckRebin (int nb) {
  bool RebinNeeded = NO, found;
  char radix[1024], filename[1024];
  int i, iold, jold, l, j, lo, loip, lojp, loipjp, type;
  double ifrac, jfrac, jreal, r, angle, dangle;
  FILE *ARR;
  double *Old_r, *OldArray, *NewArray;
  //if (!CPU_Master) return;
  ...
  OldArray = (double *)malloc(OldNRAD*OldNSEC*sizeof(double));
  NewArray = (double *)malloc(GLOBALNRAD*NSEC*sizeof(double));
  ...
  for (type = 0; type < 4; type++) {
    ...
    if (ARR != NULL) {
      fread (OldArray, sizeof(double), OldNRAD*OldNSEC, ARR);
      fclose (ARR);
      for (i = 0; i < GLOBALNRAD; i++) {
	r = New_r[i];
	iold = 0;
	...
	for (j = 0; j < NSEC; j++) {
	  l = j+i*NSEC;
	  angle = ((double)j-dangle)/(double)NSEC*2.0*M_PI;
	  jreal = angle/2.0/M_PI*(double)OldNSEC+dangle;
	  while (jreal < 0.0) jreal += (double)OldNSEC;
	  jold = (int)jreal;
	  jold = jold % OldNSEC;
	  jfrac = jreal-(double)jold;
      ...
      ARR = fopenp (filename, (char*)"w");
      fwrite (NewArray, sizeof(double), NRAD*NSEC, ARR);
      fclose (ARR);
    }
    else {
      mastererr((char*)"Could not rebin %s. File not found\n", filename);
    }
  }
}
