...
/** Definition of the type 'real' used throughout the code. You can
    ...
    double here. This is an untested feature of FARGO, however, and in
...
struct force {			
  double fx_inner;		/**< x-component of the force arising from the inner disk, without Hill sphere avoidance  */
  double fy_inner;		/**< y-component of the force arising from the inner disk, without Hill sphere avoidance  */
  double fx_ex_inner; /**< x-component of the force arising from the inner disk, with Hill sphere avoidance  */	
  double fy_ex_inner; /**< y-component of the force arising from the inner disk, with Hill sphere avoidance  */        
  double fx_outer;    /**< x-component of the force arising from the outer disk, without Hill sphere avoidance */
  double fy_outer;	  /**< y-component of the force arising from the outer disk, without Hill sphere avoidance */
  double fx_ex_outer;	/**< x-component of the force arising from the outer disk, with Hill sphere avoidance  */	
  double fy_ex_outer;	/**< x-component of the force arising from the outer disk, with Hill sphere avoidance  */        
};
...
struct polargrid {
  ...
  double           *Field;      /** Pointer to the array of Nrad*Nsec reals (e.g., density, etc.) */
  double           *gpu_field;  /**< Pointer to the array of Nrad*Nsec doubles in GPU's RAM */
  char             *Name;       /**< Name of the PolarGrid (can be "dens", "vrad", "vtheta" or "label"). */
  size_t           pitch;       /**< Pitch (stride) of 2D arrays in bytes in GPU's RAM */
  struct polargrid *next;
};
...
/** Contains all the information about a planetary system at a given
    instant in time. */
struct planetary_system {	/* Below : 'double' regardless of accuracy of hydro */
  int     nb;			        /**< Number of planets */
  double *mass;			      /**< Masses of the planets */
  double *x;			        /**< x-coordinate of the planets */
  double *y;			        /**< y-coordinate of the planets */
  double *vx;			        /**< x-coordinate of the planets'velocities */
  double *vy;		          /**< y-coordinate of the planets'velocities */
  double *acc;			      /**< The planets' accretion times^-1 */
  double *acc_rate;			  /**< The planets' accretion rate */
  char  **name;			      /**< The planets' names */
  bool   *FeelDisk;		  /**< For each planet tells if it feels the disk (ie migrates) */
  bool   *FeelOthers;	  /**< For each planet tells if it feels the other planet's gravity */
};
